...
class SkillService:
    ...
    def validate_form(self, form, image):
        required = {"student_name": "Student name", "title": "Skill title", "category": "Category", "description": "Description", "teach": "What you can teach", "learn": "What you want to learn"}
    ...
    def create_skill(self, form, image):
        image_url = self.storage.save(image) if image and image.filename else None
        return self.repository.create(Skill.new(id=str(uuid.uuid4()), student_name=form["student_name"].strip(), title=form["title"].strip(), category=form["category"].strip(), description=form["description"].strip(), teach=form["teach"].strip(), learn=form["learn"].strip(), image_url=image_url))